...
- **`marq serve`** — an **MCP stdio server** (JSON-RPC over stdin/stdout) for an external MCP client (Claude Code, Codex, Claude Desktop, etc.) that brings its own model. No network port is opened.
- **`marq run <tool> '<json>'`** — invoke one tool directly. This is the interface a terminal agent uses: a host shim (`pi/marq`) `docker exec`s into a long-lived container, and a markdown skill (`pi/SKILL.md`) teaches a local model to drive `marq run` from bash. marq owns the tools; the agent loop + model runtime are borrowed (e.g. [Pi](https://pi.dev/) on the host). `marq tools` lists the catalog.
...
- **`internal/audit`** — append-only JSON-lines log, `fsync` per write, one `invocation.start` + one `invocation.end` record per call, and each record **mirrored to stderr** (a tamper-evident copy outside the `marq` user's reach). This is the project's core safety control ("logging-only" guardrails: **scope** is recorded not enforced, but a call whose start record can't be persisted is **refused at `runner.Run`** — fail-closed, so the log can't be silently skipped; everything that runs is attributable). If hard scope-enforcement (e.g. an allowlist) is ever wanted, the place to add it is `runner.Run` before `os/exec`, since every exec funnels there.
