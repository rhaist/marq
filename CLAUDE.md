...
- **`marq tui`** — _(phases 3–4, in progress)_ a TUI agent host with its own tool-calling loop driving a host-native local model runtime (Ollama/llama.cpp). The model runs on the **host** (Apple-Silicon Metal has no Docker GPU passthrough), reached over `host.docker.internal`.
...
- **`internal/config`** — all config is env-driven (`MARQ_*`), resolved once into the package-level `config.C`. Adding a knob = add a field + env read in `Load()`. The TUI-editable fields (operator/engagement/scope/model\_\*) additionally layer a persisted file at `config.C.TUIConfigPath` (default `<WorkDir>/.marq/tui.json`, override `MARQ_TUI_CONFIG`): precedence is explicit env > file > default. `SaveTUIConfig` writes the form's values; `TUIConfigExists` lets the TUI skip setup on subsequent launches.
