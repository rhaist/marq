...
- **`internal/config`** — all config is env-driven (`MARQ_*`), resolved once into the package-level `config.C`. Adding a knob = add a field + env read in `Load()`. Operator/engagement/scope come from `MARQ_OPERATOR`/`MARQ_ENGAGEMENT`/`MARQ_SCOPE` (the `pi/marq` shim passes them through with `--env-file`).
