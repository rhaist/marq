...
- **`internal/mcpserver`** — adapts `registry.All()` onto an MCP stdio server using the official `github.com/modelcontextprotocol/go-sdk`. Each tool is added via the low-level `Server.AddTool(&mcp.Tool{InputSchema: t.InputSchema()}, handler)` with a raw-args handler that calls `t.Call`. Exposes the `pentest://authorization`, `pentest://methodology` and `pentest://skills[/<name>]` resources and the `server_info` tool (the model should call `server_info` first to confirm scope).
...
- **`internal/skills`** (feature: knowledge library) — embedded markdown playbooks (per-vuln-class + per-technique, under `library/`) tied to this server's tool names. Exposed via the `load_skill` tool (registry) and `pentest://skills` + `pentest://skills/<name>` MCP resources; the agent host also lists them in its system prompt. Add a skill = drop a `.md` with `name`/`description` frontmatter in `library/<category>/`.
